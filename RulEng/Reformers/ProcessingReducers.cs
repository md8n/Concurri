...
public static class ProcessingReducers
{
    ...
    private static ProcessingRulEngStore AllOperations(this ProcessingRulEngStore newState, RulEngStore previousState, IOpReqProcessing prescription)
    {
        ...
        var groupedDestinations = destinationEntities
            .GroupBy(de => new EntityMatch { EntityId = de.EntityId, EntType = de.EntType })
    ...
    private static ProcessingRulEngStore OperationMxProcessing(this ProcessingRulEngStore newState, RulEngStore previousState,
        ...
        List<EntityMatch> acceptableDestinations)
    {
        ...
        foreach (var opPresProc in operationprescriptionsToProcessList) {
            ...
            foreach (var opo in opPresOperands) {
                if (acceptableDestinations.None(opo)) {
                    continue;
                }
        ...
        foreach (var ruleResultIdToProcess in ruleResultIds)
        {
            ...
            foreach (var relevantOp in relevantOps)
            {
                ...
                IEntityMatch firstEnt = relevantOp.Operands[0];

                if (acceptableDestinations.None(firstEnt)) {
                    continue;
                }
                ...
                var destEntsToProcess = relevantOp.Operands
                    .Select(de => new
                    {
                        ...
                        sourceValues = de.SourceEntityIds
                            .Select(sv => JsonDocument.Parse($"{{\"Id\":\"{sv}\",\"Value\":{acceptableSources.FirstOrDefault(a => a.EntityId == sv)?.Detail.ToJsonString()}}}"))
                            .ToArray()
                    })
                    .ToList();
                ...
                foreach (var destEnt in destEntsToProcess) {
                    ...
                    foreach (Match match in regexToken.Matches(jTempl).Cast<Match>()) {
                        ...
                        JsonElement sourceVal = sourceVals[index].RootElement.GetProperty("Value");

                        jCode = jCode.Replace(token, sourceVal.ToString());
                    }
                    ...
                    // TODO: Run the Engine evaluation over the jCode to process all of the Javascript into individual values
                    JsonElement? result = null;
                    if (jCode != null && jCode.Contains("|||")) {
                        Match[] jMatches = regexJS.Matches(jCode).Cast<Match>().ToArray();
                        for (int ix = jMatches.Length - 1; ix >= 0; ix--) {
                            Match match = jMatches[ix];
                            var jsCode = match.Groups["jint"].Value;
                            var res = e.Evaluate(jsCode);
                            var st = match.Index;
                            var ln = match.Length;

                            string fullMatch = jCode.Substring(st, ln);

                            jCode = jCode.Replace(fullMatch, res.ToString());
                        }
                    }

                    result = jCode != null && ((jCode.StartsWith('{') || jCode.StartsWith('['))) ? JsonDocument.Parse(jCode).RootElement : null;
                    if (result.HasValue) {
                        ...
                        switch ((EntityType)destEnt.EntType)
                        {
                            case EntityType.Rule:
                                ...
                                newState.FromOperationResultAddUpdateRule(result.Value, destEnt.EntityId);
                                break;
                            case EntityType.Operation:
                                // Create/Update an Operation using destEnt.EntityId and result
                                newState.FromOperationResultAddUpdateOperation(result.Value, destEnt.EntityId);
                                break;
                            case EntityType.Request:
                                // Create/Update a Request using destEnt.EntityId and result
                                newState.FromOperationResultAddUpdateRequest(result.Value, destEnt.EntityId);
                                break;
                            case EntityType.Value:
                                // Create/Update a Value using destEnt.EntityId and result
                                newState.FromOperationResultAddUpdateValue(result.Value, destEnt.EntityId);
                                break;
                        }
                    }
        ...
        List<EntityMatch> acceptableDestinations)
    {
        ...
        foreach (var ruleResultIdToProcess in ruleResultIds)
        {
            ...
            foreach (var relevantOp in relevantOps)
            {
                ...
                {
                    case EntityType.Rule:
                        e.SetValue("source", JsonSerializer.Serialize(previousState.Rules.ToArray()));
                        break;
                    case EntityType.RuleResult:
                        e.SetValue("source", JsonSerializer.Serialize(previousState.RuleResults.ToArray()));
                        break;
                    case EntityType.Operation:
                        e.SetValue("source", JsonSerializer.Serialize(previousState.Operations.ToArray()));
                        break;
                    case EntityType.Request:
                        e.SetValue("source", JsonSerializer.Serialize(previousState.Requests.ToArray()));
                        break;
                    case EntityType.Value:
                        e.SetValue("source", JsonSerializer.Serialize(previousState.Values.ToArray()));
                        break;
                }

                // The result must be a list of guids for the entities that met the search criteria
                var result = e.Evaluate(relevantOp.OperationTemplate);

                List<Guid> sourceGuids = result.ToString()
    ...
    public static T GetEntityFromValue<T>(this ProcessingRulEngStore newState, OperandKey entity) where T : IEntity {
        T newEntity = default;
        Guid? refValueId = entity.SourceEntityIds.FirstOrDefault();
        if (refValueId != null) {
            JsonNode detail = newState.Values.FirstOrDefault(v => v.ValueId == refValueId).Detail;
            newEntity = JsonSerializer.Deserialize<T>(detail.ToString());
        }
